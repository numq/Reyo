package io.github.numq.reyo.drawing
...
fun DrawingCanvas(
    ...
    points: List<Offset>,
    paths: List<Path>,
    onPointsAdd: (Offset) -> Unit,
    onPointsClear: () -> Unit,
    onPathsAdd: (Path) -> Unit,
    onPathsClear: () -> Unit,
    ...
    val drawContent: DrawScope.() -> Unit = {
        ...
        if (points.isNotEmpty()) {
            drawPath(
                path = buildPath(points), color = Color.Black, style = Stroke(
                    width = strokeWidth,
                    cap = StrokeCap.Round,
